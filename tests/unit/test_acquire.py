...
def test_pipeline_run(MockDB, MockScraper, MockAPI, mocker, tmp_path):
	...
	def mock_getenv(key, default=None):
		if 'DIR' in key:
			return str(tmp_path / 'backup_dir')
		if 'CSV' in key:
	...
	api_instance.run.return_value = pd.DataFrame({'video_id': ['vid1', 'vid2'], 'comments_disabled_x': [True, False]})

	scraper_instance = MockScraper.return_value
	scraper_instance.scrape_videos.return_value = pd.DataFrame(
		{'video_id': ['vid1', 'vid2'], 'comments_disabled_y': [False, True]}
	)

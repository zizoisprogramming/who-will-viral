...
def _get_gx_results(gx_result_obj):
    """Version-safe accessor for GX result entries."""
    return getattr(gx_result_obj, "results", None) or getattr(gx_result_obj, "validation_results", [])
...
def quick_summary(df: pd.DataFrame):
    ...
    lines = []
    lines.append("\n" + "=" * 65)
    lines.append("  DATASET QUICK SUMMARY")
    lines.append("=" * 65)
    ...
    if "is_trending" in df.columns:
        ...
        for val in counts.index:
            lines.append(f"    {val} → {counts[val]} rows ({pct[val]:.1f}%)")
    ...
    for col in num.index[:8]:
        lines.append(
            f"    {col:<35}  mean={num.loc[col, 'mean']:>12.1f}  "
    ...
    path = os.path.join(VALIDATION_PATH, f"quick_summary_{timestamp}.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write(output)
    print(f"  Quick summary saved to: {path}")


def _format_gx_report(results) -> str:
    lines = []
    lines.append("=" * 65)
    lines.append("  GREAT EXPECTATIONS REPORT  (v1.x)")
    lines.append("=" * 65)
    lines.append(f"  Overall : {'PASSED' if results.success else 'FAILED'}")
    lines.append("=" * 65)

    for exp in _get_gx_results(results):
        ...
        if not exp.success and exp.result:
            ...
            if r.get("unexpected_count"):
                lines.append(f"             Issues : {r['unexpected_count']} unexpected values")
            if r.get("partial_unexpected_list"):
                lines.append(f"             Sample : {r['partial_unexpected_list'][:3]}")
...
def _print_gx_report(results):
    print(_format_gx_report(results))


def _save_gx_report(results):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = os.path.join(VALIDATION_PATH, f"gx_report_{timestamp}.txt")
    with open(path, "w", encoding="utf-8") as f:
        f.write(_format_gx_report(results))
    print(f"  GX report saved to: {path}")
...
def run_gx_validation(df: pd.DataFrame):
    ...
    for col in ["view_count", "likes", "comment_count", "favoriteCount"]:
        suite.add_expectation(gx.expectations.ExpectColumnValuesToBeBetween(column=col, min_value=0))

    # ── ACCURACY: categoryId must be between 1 and 44 (standard) ─────────
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeBetween(column="categoryId", min_value=1, max_value=44))

    # ── ACCURACY: duration must be valid ISO-8601 ────────────────
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToMatchRegex(
            column="duration", regex=r"^P(?:\d+D)?(?:T(?:\d+H)?(?:\d+M)?(?:\d+S)?)?$"
        )
    )

    # ── ACCURACY: publishedAt must start with YYYY-MM-DD ─────────
    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToMatchRegex(column="publishedAt", regex=r"^\d{4}-\d{2}-\d{2}")
    )
    ...
    suite.add_expectation(gx.expectations.ExpectCompoundColumnsToBeUnique(column_list=list(df.columns)))

    # ── UNIQUENESS: key ────────────────────────────────
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeUnique(column="video_id"))
    ...
    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(column="dimension", value_set=["2d", "3d"]))

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(column="definition", value_set=["hd", "sd"]))

    suite.add_expectation(
        gx.expectations.ExpectColumnValuesToBeInSet(column="projection", value_set=["rectangular", "360"])
    )

    # ── DISTRIBUTION: row count bounds ───────────────────────────
    suite.add_expectation(gx.expectations.ExpectTableRowCountToBeBetween(min_value=100, max_value=500_000))

    # ── DISTRIBUTION: view_count median > 0 ─────────────────────
    suite.add_expectation(gx.expectations.ExpectColumnMedianToBeBetween(column="view_count", min_value=1))

    # ── STRUCTURAL: video_id must be exactly 11 chars ────────────
    suite.add_expectation(
        gx.expectations.ExpectColumnValueLengthsToBeBetween(column="video_id", min_value=11, max_value=11)
    )
...
class DataValidator:
        ...
        for col, expected_type in expected_types.items():
            if col in df.columns:
                ...
                if actual_type != expected_type:
                    report["passed"] = False
                    report["issues"].append(f"Column '{col}': expected '{expected_type}', got '{actual_type}'")
    ...
    def validate_cross_column_rules(self, df):
        ...
        if {"is_trending", "trending_date"}.issubset(df.columns):
            ...
            if n:
                report["passed"] = False
                report["issues"].append(f"is_trending=1 but trending_date is null: {n} rows")

        # comments_disabled=True -> comment_count must be 0
        if {"comments_disabled", "comment_count"}.issubset(df.columns):
            n = df[(df["comments_disabled"].eq(True)) & (df["comment_count"] != 0)].shape[0]
            if n:
                report["passed"] = False
                report["issues"].append(f"comments_disabled=True but comment_count!=0: {n} rows")
        ...
        if {"likes", "view_count"}.issubset(df.columns):
            ...
            if n:
                report["passed"] = False
                report["issues"].append(f"likes > view_count (impossible): {n} rows")
        ...
        if {"contentDetails.contentRating.ytRating", "comments_disabled"}.issubset(df.columns):
            ...
            if n:
                report["passed"] = False
                report["issues"].append(f"Age Restricted Content but comments_disabled=False: {n} rows")
    ...
    def validate_date_order(self, df, earlier_col, later_col):
        ...
        report = self._make_report(f"Date Order ({earlier_col} <= {later_col})", "Timeliness")
        if earlier_col not in df.columns or later_col not in df.columns:
            report["issues"].append(f"Column(s) not found: '{earlier_col}', '{later_col}'")
            return self._save(report)
        ...
        unparseable = (df[earlier_col].notna() & t1.isna()).sum() + (df[later_col].notna() & t2.isna()).sum()
        if unparseable:
            report["issues"].append(f"{unparseable} non-null values could not be parsed as dates")

        n = (t1.notna() & t2.notna() & (t2 < t1)).sum()
        if n:
            report["passed"] = False
            report["issues"].append(f"{n} rows where '{later_col}' is earlier than '{earlier_col}'")
        return self._save(report)
    ...
    def validate_outliers_iqr(self, df, numeric_columns, multiplier=1.5):
        ...
        for col in numeric_columns:
            ...
            if n:
                report["passed"] = False
                report["issues"].append(f"Column '{col}': {n} extreme outliers (bounds [{lo:.0f}, {hi:.0f}])")
        return self._save(report)
    ...
    def validate_outliers_zscore(self, df, numeric_columns, threshold=5.0):
        ...
        for col in numeric_columns:
            ...
            if n:
                report["passed"] = False
                report["issues"].append(f"Column '{col}': {n} values with |z| > {threshold}")
        return self._save(report)
    ...
    def validate_category_dominance(self, df, col, max_share=0.80):
        ...
        if top_share > max_share:
            report["passed"] = False
            report["issues"].append(f"Top value covers {top_share:.1%} (max allowed: {max_share:.0%})")
        else:
    ...
    def validate_non_zero_variance(self, df, numeric_columns):
        ...
        for col in numeric_columns:
            ...
            if df[col].std() == 0:
                report["passed"] = False
                report["issues"].append(f"Column '{col}': all values are identical (zero variance)")
        return self._save(report)
    ...
    def validate_class_imbalance(
        ...
        if imbalance_ratio > threshold:
            report["passed"] = False
            report["issues"].append(f"Severe class imbalance: dominant class = {imbalance_ratio:.1%}")
        else:
            p1 = counts.get(1, 0.0)
            p0 = counts.get(0, 0.0)
    ...
    def validate_skew(self, df, numeric_columns, skew_threshold=1):
        ...
        for col in numeric_columns:
            ...
            if abs(skew_val) > skew_threshold:
                report["issues"].append(f"Skewness {skew_val:.2f} exceeds threshold {skew_threshold} in '{col}'")
                report["passed"] = False
                found_issue = True
    ...
    def validate_count_matches_list(self, df, count_col, list_col):
        ...
        report = self._make_report(f"Count Match: '{count_col}' vs len('{list_col}')", "Referential Integrity")
        if count_col not in df.columns or list_col not in df.columns:
            report["issues"].append(f"Column(s) not found: '{count_col}', '{list_col}'")
            return self._save(report)
        ...
        mismatch = (actual.ge(0) & (df[count_col].fillna(0).astype(int) != actual)).sum()

        if parse_errors:
            report["passed"] = False
            report["issues"].append(f"{parse_errors} rows in '{list_col}' could not be parsed as a list")

        if mismatch:
            report["passed"] = False
            report["issues"].append(f"{mismatch} rows where '{count_col}' != len('{list_col}')")
        return self._save(report)
    ...
    def generate_report(self):
        ...
        print("\n" + "=" * 65)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = os.path.join(VALIDATION_PATH, f"pandas_report_{timestamp}.txt")
    ...
    def _format_report(self) -> str:
        """Same content as generate_report prints, but returns a string."""
        lines = []
        lines.append("=" * 65)
        lines.append("  PANDAS VALIDATION REPORT")
        lines.append("=" * 65)
        for result in self.validation_results:
            status = "PASS" if result["passed"] else "FAIL"
            lines.append(f"\n[{status}]  [{result['dimension']}]  {result['check_type']}")

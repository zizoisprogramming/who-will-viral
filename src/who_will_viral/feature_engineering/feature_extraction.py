...
RATIOS = {'likes': 'like_to_view_ratio', 'comment_count': 'comment_to_view_ratio'}
...
class FeatureExtraction:
    ...
    def _count_emojis(self, df):
        for col in EMOJIS_COLS:
            df[f'{col}_emoji_count'] = df[col].apply(lambda x: len(emoji.emoji_list(x)) if isinstance(x, str) else 0)
        return df

...
if __name__ == '__main__':
    ...
    bars = ax.bar(
        counts.index.astype(str), counts.values, color=sns.color_palette('muted', 2), edgecolor='white', linewidth=0.8
    )
    ...
    sns.heatmap(corr, mask=mask, annot=True, fmt='.2f', cmap='coolwarm', center=0, linewidths=0.5, square=True, ax=ax)
    ...
    for i, col in enumerate(CATEGORICAL_COLUMNS):
        ax = axes[i]
        rate = df.groupby(col)[TARGET_COLUMN].mean().sort_values(ascending=False).head(15)
    ...
    for ax, col in zip(axes, RATIO_COLS, strict=True):
        for label, grp in df.groupby(TARGET_COLUMN):
            ...
            sns.kdeplot(vals, ax=ax, fill=True, alpha=0.4, label=f'trending={label}', color=palette[label])
        ax.set_title(col)
    ...
    df['title_has_emoji'] = df['title'].apply(
        lambda x: bool(any(ord(c) > 127 and ord(c) < 0x1F9FF or ord(c) > 0x1F300 for c in str(x)))
    )
    df['title_has_question'] = df['title'].str.contains(r'\?', regex=True)
    df['title_has_hashtag'] = df['title'].str.contains(r'#', regex=True)
    df['title_has_caps'] = df['title'].apply(lambda x: sum(c.isupper() for c in str(x)) / (len(str(x)) + 1) > 0.3)
    df['title_len_bin'] = pd.cut(
        df['title_len'], bins=[0, 20, 40, 60, 80, 200], labels=['≤20', '21-40', '41-60', '61-80', '>80']
    )
    ...
    flag_df.plot(kind='bar', ax=axes[1], color=['#5b84b1', '#fc5c65'], edgecolor='white')
    ...
    axes[0].annotate(
        ...
        xy=(10, top10_pct),
        xytext=(25, top10_pct - 20),
        arrowprops=dict(arrowstyle='->', color='gray'),
        fontsize=8,
    )

    ver_rate = df.groupby('is_verified')[TARGET_COLUMN].mean()
    ver_rate.plot(kind='bar', ax=axes[1], color=['#5b84b1', '#fc5c65'], edgecolor='white', width=0.5)
    ...
    # ── Temporal Patterns ─────────────────────────────────────────────────────────
    pivot_time = df.pivot_table(values=TARGET_COLUMN, index='hour', columns='weekday', aggfunc='mean')
    ...
    sns.heatmap(pivot_time, cmap='YlOrRd', annot=False, linewidths=0.3, ax=axes[0], cbar_kws={'label': 'P(trending)'})
    ...
    for col_idx, day in enumerate(pivot_time.columns):
        ...
        axes[0].add_patch(plt.Rectangle((col_idx, row_idx), 1, 1, fill=False, edgecolor='#2c3e50', lw=2))

    monthly = df.groupby('month').agg(trending_rate=(TARGET_COLUMN, 'mean'), video_count=('video_id', 'count'))
    ax2 = axes[1]
    ax2b = ax2.twinx()

    ax2.bar(monthly.index, monthly['video_count'], color='#5b84b1', alpha=0.4, label='video count')
    ax2b.plot(monthly.index, monthly['trending_rate'], color='#fc5c65', marker='o', lw=2, label='trending rate')
    ...
    fig.add_artist(
        plt.Rectangle((0.038, 0.962), 0.003, 0.030, transform=fig.transFigure, color=PALETTE['red'], clip_on=False)
    )
    fig.text(
        0.044,
        0.982,
    ...
    gs = fig.add_gridspec(3, 4, top=0.95, bottom=0.07, hspace=0.40, wspace=0.35, height_ratios=[0.7, 2.5, 1.5])
    ...
    for i, ((title, value), bg, acc) in enumerate(zip(kpis.items(), kpi_bg, kpi_accent, strict=True)):
        ax = fig.add_subplot(gs[0, i])
        ax.set_facecolor(bg)
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.axis('off')
        ax.text(0.5, 0.62, value, ha='center', va='center', fontsize=17, fontweight='bold', color=acc)
        ax.text(0.5, 0.24, title, ha='center', va='center', fontsize=9, color=PALETTE['subtext'])
    ...
    quarter_months = {1: 'Jan · Feb · Mar', 2: 'Apr · May · Jun', 3: 'Jul · Aug · Sep', 4: 'Oct · Nov · Dec'}
    ...
    ax5.plot(mean_vals.index, mean_vals.values, lw=2.5, zorder=3, color=PALETTE['text'], marker='o', ms=0)

    for q, v in mean_vals.items():
        ax5.scatter(q, v, s=120, color=quarter_colors[q - 1], zorder=4, edgecolors='white', linewidths=1.5)

    ax5.fill_between(mean_vals.index, mean_vals.values, alpha=0.06, color=PALETTE['text'], zorder=2)

    for q, v in mean_vals.items():
        ax5.text(q, v + 0.003, f'{v:.1%}', ha='center', fontsize=9, fontweight='bold', color=quarter_colors[q - 1])
    ...
    for i in range(1, len(mean_vals)):
        ...
        ax5.text(mid_x, mid_y + 0.005, f'{arrow} {abs(delta):.1%}', ha='center', fontsize=7.5, color=col)
    ...
    for (label, grp), col in zip(df.groupby(TARGET_COLUMN), kde_colors, strict=True):
        sns.kdeplot(
            np.log1p(grp['comment_rate'].clip(lower=0)),
            ax=ax6,
            fill=True,
            alpha=0.35,
            color=col,
    ...
    for (label, grp), col in zip(df.groupby(TARGET_COLUMN), kde_colors, strict=True):
        sns.kdeplot(
            np.log1p(grp['description_len'].clip(lower=0)),
            ax=ax1,
            fill=True,
            alpha=0.35,
            color=col,
    ...
    flag_df = pd.DataFrame(flag_rates).T
    flag_df.plot(
        kind='bar', ax=ax2, color=[PALETTE['light_gray'], PALETTE['light_blue']], edgecolor='white', width=0.65
    )
    ...
    for container in ax2.containers:
        ax2.bar_label(container, fmt='{:.1%}'.format, label_type='edge', fontsize=7, padding=2)

    # Trending rate by hour
    ax3 = fig.add_subplot(gs[2, 2])
    hour_rate = df.groupby('hour')[TARGET_COLUMN].mean()
    ax3.plot(hour_rate.index, hour_rate.values, marker='o', ms=4, color=PALETTE['blue'], lw=2)
    ax3.fill_between(hour_rate.index, hour_rate.values, alpha=0.12, color=PALETTE['blue'])
    ...
    ranked['cum_pct_channels'] = np.arange(1, len(ranked) + 1) / len(ranked) * 100
    ranked['cum_pct_trending'] = ranked['trending_count'].cumsum() / ranked['trending_count'].sum() * 100
    ax4.plot(ranked['cum_pct_channels'], ranked['cum_pct_trending'], lw=2, color=PALETTE['blue'])
    ax4.plot([0, 100], [0, 100], '--', lw=1, color=PALETTE['gray'], label='perfect equality')
    ax4.fill_between(
    ...
    top10_pct = ranked.head(int(len(ranked) * 0.1))['trending_count'].sum() / ranked['trending_count'].sum() * 100
    ax4.annotate(
        f'Top 10% → {top10_pct:.0f}%',
        xy=(10, top10_pct),
        xytext=(30, top10_pct - 20),
